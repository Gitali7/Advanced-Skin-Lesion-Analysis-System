...
import torch
# Monkey patch for torch.solve (removed in torch >= 1.9)
...
def model_predict(img):
    ...
    # Calculate probabilities as floats
    probs = [x * 100 for x in torch.nn.functional.softmax(outputs, dim=0)]
    
    # Zip with classes and sort by probability descending (numerically)
    class_probs = sorted(
        zip(learn.data.classes, probs),
        key=lambda p: p[1],
        reverse=True
    )
    
    # Format as strings for display
    pred_probs = [(c, "{:.1f}%".format(p)) for c, p in class_probs]
